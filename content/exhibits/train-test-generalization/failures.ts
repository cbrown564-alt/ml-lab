  ...
  cards: [
    {
      ...
      trigger: "Hold out only a handful of points and report the test error from that one split.",
      ...
      diagnosis: "A small test set is a tiny, noisy sample of the model's true error, so any single number off it is dominated by which points happened to land in the holdout.",
      repair: "Use cross-validation (average over k folds) so every point is tested once and the luck averages out — or hold out more data if you can spare it.",
      boundary: "With a large test set, a single split is already stable — the lottery is specifically a small-holdout problem.",
    },
    {
      id: "tuning-on-test",
