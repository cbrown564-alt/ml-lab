...
export const whatIsMlNarrative: ExhibitNarrative = {
  nodeId: "what-is-ml",
  hook: [
    "For seventy years, programming meant one thing: a human writes the rules, the computer follows them exactly. It works brilliantly — when you can spell the rules out. But write the rule for “is this a cat?”, or “is this email spam?”, or “what will this house sell for?” and you'll stall. Nobody can. Machine learning is the move that gets unstuck: instead of writing the rule, you show examples and let the machine write it.",
  ...
  story: [
    ...
    },
    {
      ...
      paragraphs: [
        "So don't write it — show it. Hand the machine the labelled examples (each point with its known class) and it searches for the rule that fits them best, free to weigh every feature at once. Here it finds the tilted line, using both features together, and beats your hand-tuned cut. The same move scales to patterns no human could spell out: the machine isn't following a rule you wrote, it's discovering one from the data.",
      ],
    },
    {
      id: "the-inversion",
      heading: "The whole field, in one inversion",
      paragraphs: [
        "That's the definition, and it's an inversion. Traditional programming takes rules and data and produces answers. Machine learning takes data and answers — examples — and produces the rule. You stop telling the computer how to do the task and start showing it what done looks like. Everything else in this lab is a way of doing that search well: which rules to consider, how to measure fit, how to find the best one.",
      ],
    },
    {
  ...
  fieldNotes: [
    "This exhibit is the doorway to every other one. “Which rules to consider” is the model (linear regression, a neural network); “how to measure fit” is the loss; “how to find the best one” is gradient descent; “does it work on new data” is generalisation. They're all pieces of learning a rule from examples.",
    "The labels matter. Showing the machine inputs with their correct answers is supervised learning — the kind here. Show it inputs with no answers and it can only find structure (clusters, say), not a labelled rule. The answers in your examples are what let it learn what you actually want.",
  ],
};
