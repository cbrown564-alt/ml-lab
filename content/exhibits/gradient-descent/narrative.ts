...
export const gradientDescentNarrative: ExhibitNarrative = {
  ...
  story: [
    ...
    },
    {
      ...
      heading: "Too timid",
      paragraphs: [
        "Now make the stride tiny. The gradient still points the right way, but at this learning rate the walker only shuffles — the dot barely leaves where it began, and thousands of steps would pass before it reached the valley. In this scenario, the model is progressing but the step is too small to finish in a practical number of iterations.",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "That runaway has a name — divergence — and it is one way a training run can fail when the step size is too large. The fix is humbling: take smaller steps.",
      ],
    },
    {
      id: "the-one-knob",
      ...
      paragraphs: [
        ...
        "That is why the learning rate is the first hyperparameter every practitioner learns to respect — and often the first one they tune. A flat loss curve can also signal poor conditioning, vanishing gradients, noisy mini-batches, or a representational limit, not just a step size that is too small. The same fog, the same slope, the same step scale up untouched to models with billions of parameters.",
      ],
    },
  ],
  fieldNotes: [
    ...
    "When a training run diverges and the loss curve rockets upward, step size is one of the first suspects — but also inspect gradient norms, feature scaling, and conditioning before assuming the learning rate alone is at fault.",
  ],
};
